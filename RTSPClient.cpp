...
: fNumAddresses(0), fAddressArray(NULL) {
  ...
  igtl_uint32 addr = inet_addr((const char*)hostname);
  if (addr != INADDR_NONE) {
    ...
    fAddressArray[0] = new NetAddress((u_int8_t*)&addr, sizeof (igtl_uint32));
    return;
  }
...
RTSPClient* RTSPClient::createNew(char const* rtspURL,
				  int verbosityLevel,
				  char const* applicationName,
				  igtl_uint16 tunnelOverHTTPPortNum,
				  int socketNumToServer) {
  return new RTSPClient(rtspURL,
...
unsigned RTSPClient::sendAnnounceCommand(char const* sdpDescription, responseHandler* responseHandler, Authenticator* authenticator) {
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
  return sendRequest(new RequestRecord(++fCSeq, "ANNOUNCE", responseHandler,false, 0.0, 0.0, 0.0, sdpDescription));
}

unsigned RTSPClient::sendSetupCommand( responseHandler* responseHandler,
                                      bool streamOutgoing, bool streamUsingTCP, bool forceMulticastOnUnspecified,
				      Authenticator* authenticator) {
  if (fTunnelOverHTTPPortNum != 0) streamUsingTCP = true; // RTSP-over-HTTP tunneling uses TCP (by definition)
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;

  u_int32_t boolFlags = 0;
  if (streamUsingTCP) boolFlags |= 0x1;
  if (streamOutgoing) boolFlags |= 0x2;
  if (forceMulticastOnUnspecified) boolFlags |= 0x4;
  return sendRequest(new RequestRecord(++fCSeq, "SETUP", responseHandler, boolFlags));
}

unsigned RTSPClient::sendPlayCommand( responseHandler* responseHandler,
                                     double start, double end, float scale,
                                     Authenticator* authenticator) {
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
  sendDummyUDPPackets(); // hack to improve NAT traversal
  return sendRequest(new RequestRecord(++fCSeq, "PLAY", responseHandler, 0, start, end, scale));
}
...
unsigned RTSPClient::sendPlayCommand( responseHandler* responseHandler,
  ...
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
  sendDummyUDPPackets(); // hack to improve NAT traversal
  return sendRequest(new RequestRecord(++fCSeq, responseHandler, absStartTime, absEndTime, scale));
}

unsigned RTSPClient::sendPauseCommand( responseHandler* responseHandler, Authenticator* authenticator) {
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
  return sendRequest(new RequestRecord(++fCSeq, "PAUSE", responseHandler ));
}

unsigned RTSPClient::sendRecordCommand( responseHandler* responseHandler, Authenticator* authenticator) {
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
  return sendRequest(new RequestRecord(++fCSeq, "RECORD", responseHandler));
}

unsigned RTSPClient::sendTeardownCommand( responseHandler* responseHandler, Authenticator* authenticator) {
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
  return sendRequest(new RequestRecord(++fCSeq, "TEARDOWN", responseHandler));
}

unsigned RTSPClient::sendSetParameterCommand( responseHandler* responseHandler,
  ...
  unsigned result = sendRequest(new RequestRecord(++fCSeq, "SET_PARAMETER", responseHandler, false, 0.0, 0.0, 0.0, paramString));
  delete[] paramString;
  return result;
}

unsigned RTSPClient::sendGetParameterCommand( responseHandler* responseHandler, char const* parameterName,
                                             Authenticator* authenticator) {
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
  ...
  unsigned result = sendRequest(new RequestRecord(++fCSeq, "GET_PARAMETER", responseHandler, false, 0.0, 0.0, 0.0, paramString));
  delete[] paramString;
  return result;
}

void RTSPClient::sendDummyUDPPackets( unsigned numDummyPackets) {
...
void RTSPClient::setSpeed( float speed) { 
...
bool RTSPClient::changeResponseHandler(unsigned cseq, responseHandler* newResponseHandler) { 
...
bool RTSPClient::lookupByName(
  ...
  char const* rtspURL = "";
  char const* applicationName = "default";
  igtl_uint16 tunnelOverHTTPPortNum = 18944;
  int socketNumToServer = 1;
  resultClient = new RTSPClient(rtspURL, 0 , applicationName ,tunnelOverHTTPPortNum , socketNumToServer);
  return true;
}
...
bool RTSPClient::parseRTSPURL(char const* url,
  ...
  do {
	...
	*to = '\0';
	...
	break;
      ...
      portNum = (igtl_uint16)portNumInt;
...
RTSPClient::RTSPClient(char const* rtspURL,
		       int verbosityLevel, char const* applicationName,
                       igtl_uint16 tunnelOverHTTPPortNum, int socketNumToServer):
  ...
  resetResponseBuffer();
      tcpSocket = igtl::ServerSocket::New();
      udpClientSocket = igtl::UDPClientSocket::New();
  ...
  char const* const libVersionStr = "IGTLink ";
...
RTSPClient::~RTSPClient() {
  //RTPInterface::clearServerRequestAlternativeByteHandler(fInputSocketNum); // in case we were receiving RTP-over-TCP
  reset();
...
void RTSPClient::setBaseURL(char const* url) {
  delete[] fBaseURL; fBaseURL = igtl::strDup(url);
}
...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
  ...
  do {
    ...
    if (fVerbosityLevel >= 1) std::cerr << "Sending request: " << cmd << "\n";
    ...
    if (fTunnelOverHTTPPortNum != 0 && strcmp(request->commandName(), "GET") != 0 && strcmp(request->commandName(), "POST") != 0) {
      ...
      char* origCmd = cmd;
      cmd = igtl::base64Encode(origCmd, strlen(cmd));
      if (fVerbosityLevel >= 1) std::cerr << "\tThe request was base-64 encoded to: " << cmd << "\n\n";
      delete[] origCmd;
    }
...
bool RTSPClient::setRequestFields(RequestRecord* request,
  ...
  if (strcmp(request->commandName(), "DESCRIBE") == 0) {
    ...
    bool streamUsingTCP = (request->boolFlags()&0x1) != 0;
    bool streamOutgoing = (request->boolFlags()&0x2) != 0;
    bool forceMulticastOnUnspecified = (request->boolFlags()&0x4) != 0;
    
    char const *prefix, *separator, *suffix;
    constructSubsessionURL(prefix, separator, suffix);
    
    char const* transportFmt;
    char* protocolName = "UDP";
    ...
    igtl_uint16 rtpNumber, rtcpNumber;
    if (streamUsingTCP) { // streaming over the RTSP connection
      ...
      const char* connectionAddress = "127.0.0.1";//subsession.connectionEndpointAddress();
      bool requestMulticastStreaming
	= IsMulticastAddreesValid(connectionAddress) || (connectionAddress == 0 && forceMulticastOnUnspecified);
      transportTypeStr = requestMulticastStreaming ? ";multicast" : ";unicast";
      portTypeStr = requestMulticastStreaming ? ";port" : ";client_port";
      rtpNumber = 18945;//subsession.clientPortNum();
      if (rtpNumber == 0) {
    ...
    delete[] username;
    delete[] password;
    igtl_uint32 serverAddress = *(igtl_uint32*)(destAddress.data());
    igtl::AddressString serverAddressString(serverAddress);
    ...
    if (strcmp(request->commandName(), "PLAY") == 0) {
      ...
      float speed = 1.0;//request->session() != NULL ? request->session()->speed() : request->subsession()->speed();
...
bool RTSPClient::isRTSPClient() const {
  return true;
}
...
int RTSPClient::openConnection() {
  do {
    ...
    igtl_uint16 destPortNum = fTunnelOverHTTPPortNum == 0 ? urlPortNum : fTunnelOverHTTPPortNum;
    ...
    fInputSocketNum = fOutputSocketNum = tcpSocket->CreateServer(18944);
    if (fInputSocketNum < 0) break;
#ifdef SO_NOSIGPIPE
    int set_option = 1;
    setsockopt(fOutputSocketNum, SOL_SOCKET, SO_NOSIGPIPE, &set_option, sizeof set_option);
#else
    signal(SIGPIPE, SIG_IGN);
#endif // so that servers on the same host that get killed don't also kill us
      
    // Connect to the remote endpoint:
    fServerAddress = *(igtl_uint32*)(destAddress.data());
...
int RTSPClient::connectToServer(int socketNum, igtl_uint16 remotePortNum) {
  struct sockaddr_in remoteName;
  remoteName.sin_family = AF_INET;\
  remoteName.sin_addr.s_addr = (fServerAddress);
  remoteName.sin_port = htons(remotePortNum);
  remoteName.sin_len = sizeof remoteName;
  if (fVerbosityLevel >= 1) {
    std::cerr << "Opening connection to " << igtl::AddressString(remoteName).val() << ", port " << remotePortNum << "...\n";
  }
  if (connect(socketNum, (struct sockaddr*) &remoteName, sizeof remoteName) != 0) {
    ...
    if (fVerbosityLevel >= 1) std::cerr << "..." <<"\n";
    return -1;
  }
  if (fVerbosityLevel >= 1) std::cerr << "...local connection opened\n";
...
char* RTSPClient::createAuthenticatorString(char const* cmd, char const* url) {
  ...
  if (auth.realm() != NULL && auth.username() != NULL && auth.password() != NULL) {
	...
	+ strlen(auth.nonce()) + strlen(url) + strlen(response);
      ...
      char* response = igtl::base64Encode(usernamePassword, usernamePasswordLength);
...
char* RTSPClient::createBlocksizeString(bool streamUsingTCP) {
  char* blocksizeStr;
...
void RTSPClient::handleRequestError(RequestRecord* request) {
  ...
  if (request->handler() != NULL) (*request->handler())(this, resultCode, "void RTSPClient::handleRequestError()");
}
...
void RTSPClient::handleIncomingRequest() {
  ...
  if (!parseRTSPRequestString(fResponseBuffer, fResponseBytesAlreadySeen,
    ...
    if (fVerbosityLevel >= 1) {
      std::cerr << "Received incoming RTSP request: " << fResponseBuffer << "\n";
    }
...
bool RTSPClient::checkForHeader(char const* line, char const* headerName, unsigned headerNameLength, char const*& headerParams) {
...
bool RTSPClient::parseTransportParams(char const* paramsStr,
  ...
  igtl_uint16 multicastPortNumRTP, multicastPortNumRTCP;
  ...
  char* field = igtl::strDupSize(fields);
  while (sscanf(fields, "%[^;]", field) == 1) {
    if (sscanf(field, "server_port=%hu", &serverPortNum) == 1) {
      ...
      foundServerAddressStr = igtl::strDup(field+7);
      ...
      foundDestinationStr = igtl::strDup(field+12);
...
bool RTSPClient::parseScaleParam(char const* paramStr, float& scale) {
  Locale l("C", Numeric);
  return sscanf(paramStr, "%f", &scale) == 1;
}

bool RTSPClient::parseSpeedParam(char const* paramStr, float& speed) {
  Locale l("C", Numeric);
  return sscanf(paramStr, "%f", &speed) >= 1;
}

bool RTSPClient::parseRTPInfoParams(char const*& paramsStr, u_int16_t& seqNum, u_int32_t& timestamp) {
  ...
  char* field = igtl::strDupSize(paramsStr);
...
bool RTSPClient::handleSETUPResponse( char const* sessionParamsStr, char const* transportParamsStr,
                                        bool streamUsingTCP) {
  ...
  do {
    ...
    delete[] fLastSessionId; fLastSessionId = igtl::strDup(sessionId);
	...
	increaseReceiveBufferTo(envir(), fInputSocketNum, 50*1024);
      ...
      igtl_uint32 destAddress = subsession.connectionEndpointAddress();
...
bool RTSPClient::handlePLAYResponse(char const* scaleParamsStr, char const* speedParamsStr,
                                       char const* rangeParamsStr, char const* rtpInfoParamsStr) {
  bool scaleOK = false, rangeOK = false, speedOK = false;
  do {
    /*if (&session != NULL) {
...
bool RTSPClient::handleTEARDOWNResponse() {
  // Because we don't expect to always get a response to "TEARDOWN", we don't need to do anything if we do get one:
  return true;
}

bool RTSPClient::handleGET_PARAMETERResponse(char const* parameterName, char*& resultValueString, char* resultValueStringEnd) {
  do {
...
bool RTSPClient::handleAuthenticationFailure(char const* paramsStr) {
  ...
  char* realm = igtl::strDupSize(paramsStr);
  char* nonce = igtl::strDupSize(paramsStr);
  char* stale = igtl::strDupSize(paramsStr);
  bool success = true;
...
bool RTSPClient::resendCommand(RequestRecord* request) {
  if (fVerbosityLevel >= 1) std::cerr << "Resending...\n";
  if (request != NULL && strcmp(request->commandName(), "GET") != 0) request->cseq() = ++fCSeq;
  return sendRequest(request) != 0;
}

char const* RTSPClient::sessionURL() const {
...
void RTSPClient::constructSubsessionURL(char const*& prefix,
...
bool RTSPClient::setupHTTPTunneling1() {
  ...
  if (fVerbosityLevel >= 1) {
    std::cerr << "Requesting RTSP-over-HTTP tunneling (on port " << fTunnelOverHTTPPortNum << ")\n\n";
  }
...
void RTSPClient::responseHandlerForHTTP_GET1(int responseCode, char* responseString) {
  ...
  do {
    ...
    fOutputSocketNum = tcpSocket->CreateServer(18944);
...
bool RTSPClient::setupHTTPTunneling2() {
  fHTTPTunnelingConnectionIsPending = false;
...
void RTSPClient::connectionHandler1() {
  ...
  do {
    int errtemp = 0;
    int lentemp = sizeof errtemp;
    if (errtemp != 0) {
      //envir().setResultErrMsg("Connection to server failed: ", errtemp);
      if (fVerbosityLevel >= 1) std::cerr << "..." << "\n";
      break;
    }

    // The connection succeeded.  If the connection came about from an attempt to set up RTSP-over-HTTP, finish this now:
    if (fVerbosityLevel >= 1) std::cerr << "...remote connection opened\n";
...
void RTSPClient::incomingDataHandler1() {
  ...
  udpClientSocket->SetIPAddress("127.0.0.1");
  udpClientSocket->SetPortNumber(18945);
  int bytesRead = udpClientSocket->ReadSocket((unsigned char*)&fResponseBuffer[fResponseBytesAlreadySeen], fResponseBufferBytesLeft);
  //int bytesRead = readSocket(fInputSocketNum, (unsigned char*)&fResponseBuffer[fResponseBytesAlreadySeen], fResponseBufferBytesLeft, dummy);
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  do {
    ...
    if (newBytesRead >= (int)fResponseBufferBytesLeft) {
      ...
      std::cerr<<"RTSP response was truncated. Increase \"RTSPClient::responseBufferSize\"";
    }
  ...
  if (fVerbosityLevel >= 1 && newBytesRead > 1) std::cerr << "Received " << newBytesRead << " new bytes of response data.\n";
  ...
  do {
	...
	if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
	  ...
	  while ((request = fRequestsAwaitingResponse.dequeue()) != NULL) {
	    if (request->cseq() < cseq) { // assumes that the CSeq counter will never wrap around
          ...
          std::cerr << "WARNING: The server did not respond to our \"" << request->commandName() << "\" request (CSeq: "
	...
	if (fVerbosityLevel >= 1) {
    std::cerr << "Have received " << fResponseBytesAlreadySeen << " total bytes of a "
	...
	*responseEnd = '\0';
  std::cerr << "Received a complete "
		<< (foundRequest != NULL ? foundRequest->commandName() : "(unknown)")
		<< " response:\n" << fResponseBuffer << "\n";
	if (numExtraBytesAfterResponse > 0) std::cerr << "\t(plus " << numExtraBytesAfterResponse << " additional bytes)\n";
	*responseEnd = saved;
      }
	...
	if (responseCode == 200) {
	  ...
	  if (strcmp(foundRequest->commandName(), "SETUP") == 0) {
	    if (!handleSETUPResponse(sessionParamsStr, transportParamsStr, foundRequest->boolFlags()&0x1)) break;
	  } else if (strcmp(foundRequest->commandName(), "PLAY") == 0) {
	    if (!handlePLAYResponse(scaleParamsStr, speedParamsStr, rangeParamsStr, rtpInfoParamsStr)) break;
	...
	char saved = *responseEnd;
	*responseEnd = '\0';
	bodyStart = igtl::strDup(bodyStart);
	*responseEnd = saved;
      }
	...
	if (responseCode == 200) {
	  resultCode = 0;
	  resultString = numBodyBytes > 0 ? igtl::strDup(bodyStart) : igtl::strDup(publicParamsStr);
          // Note: The "igtl::strDup(bodyStart)" call assumes that the body is encoded without interior '\0' bytes
	} else {
	  resultCode = responseCode;
	  resultString = igtl::strDup(responseStr);
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, char const* commandName, responseHandler* handler, u_int32_t boolFlags,
					 double start, double end, float scale, char const* contentStr)
  : fNext(NULL), fCSeq(cseq), fCommandName(commandName), fboolFlags(boolFlags),
    fStart(start), fEnd(end), fAbsStartTime(NULL), fAbsEndTime(NULL), fScale(scale), fContentStr(igtl::strDup(contentStr)), fHandler(handler) {
}

RTSPClient::RequestRecord::RequestRecord(unsigned cseq, responseHandler* handler,
					 char const* absStartTime, char const* absEndTime, float scale)
  : fNext(NULL), fCSeq(cseq), fCommandName("PLAY"), fboolFlags(0),
    fStart(0.0f), fEnd(-1.0f), fAbsStartTime(igtl::strDup(absStartTime)), fAbsEndTime(igtl::strDup(absEndTime)), fScale(scale),
...
using igtl::Port;
using igtl::UserAuthenticationDatabase;
HandlerServerForREGISTERCommand* HandlerServerForREGISTERCommand
::createNew(onRTSPClientCreationFunc* creationFunc, Port ourPort,
	    UserAuthenticationDatabase* authDatabase, int verbosityLevel, char const* applicationName) {
  return new HandlerServerForREGISTERCommand(creationFunc, ourPort, authDatabase, verbosityLevel, applicationName);
}

HandlerServerForREGISTERCommand
::HandlerServerForREGISTERCommand(onRTSPClientCreationFunc* creationFunc, Port ourPort,
                                  UserAuthenticationDatabase* authDatabase, int verbosityLevel, char const* applicationName)
  : RTSPServer(ourPort, authDatabase, 30/*small reclamationTestSeconds*/),
    fCreationFunc(creationFunc), fVerbosityLevel(verbosityLevel), fApplicationName(igtl::strDup(applicationName)) {
}
...
::createNewRTSPClient(char const* rtspURL, int verbosityLevel, char const* applicationName, int socketNumToServer) {
  ...
  return RTSPClient::createNew(rtspURL, verbosityLevel, applicationName, 0, socketNumToServer);
}
...
bool HandlerServerForREGISTERCommand
